...
int ggml_metal_op_diag_mask(ggml_metal_op_t ctx, int idx) {
    ...
    // same kernel for both ops; only the fill value differs
    const float value = op->op == GGML_OP_DIAG_MASK_INF ? -INFINITY : 0.0f;
    ...
    auto pipeline = ggml_metal_library_get_pipeline_diag_mask(lib, op);
